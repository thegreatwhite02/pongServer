...
def playGame(screenWidth:int, screenHeight:int, playerPaddle:str, client:socket.socket) -> None:
    ...
    while True:
        ...
        game_state = {
            "playerPaddle": playerPaddleObj,
            "paddleMovement": playerPaddleObj.moving,
            "playerPaddleY": playerPaddleObj.rect.y,
        ...
        # Send your server update here at the end of the game loop to sync your game with your
        # opponent's game
        # Prepare an empty dictionary to receive the game state from the server
        received_game_state = {}

        # Receive and decode the JSON string from the server
        try:
            msg = client.recv(1024).decode()
            received_game_state = json.loads(msg)
        except json.JSONDecodeError:
        ...
        if "playerPaddleY" in received_game_state:
            opponentPaddleObj.rect.y = received_game_state["playerPaddleY"]
        if "opponentPaddleY" in received_game_state:
            playerPaddleObj.rect.y = received_game_state["opponentPaddleY"]
        if "ballX" in received_game_state and "ballY" in received_game_state:
            ball.rect.x = received_game_state["ballX"]
            ball.rect.y = received_game_state["ballY"]
        if "lScore" in received_game_state:
